...
async function api(req,res){
  ...
  if(parts[1]==='orders'){
    ...
    if(method==='DELETE'&&parts[2]){const index=db.orders.findIndex(item=>item.id===parts[2]);if(index<0)return json(res,404,{error:'OS não encontrada.'});const order=db.orders[index];if(!canAccess(user,order.ownerId,'orders','edit',order.id))return json(res,403,{error:'Sem permissão para excluir esta OS.'});db.orders.splice(index,1);db.finance=db.finance.filter(item=>item.orderId!==order.id);db.shares=db.shares.filter(item=>!(item.resource==='orders'&&item.resourceId===order.id));audit(user,'excluiu OS',`#${orderNumber(order.number)}`);saveDB(db);return json(res,200,{ok:true});}
  ...
  if(['clients','stock','finance'].includes(parts[1])){
    ...
    if(method==='PUT'&&parts[2]){const item=db[collection].find(x=>x.id===parts[2]);if(!item||!canAccess(user,item.ownerId,collection,'edit'))return json(res,403,{error:'Sem permissão para editar.'});const b=await body(req);Object.assign(item,b,{id:item.id,ownerId:item.ownerId,updatedAt:now()});if(collection==='clients')db.vehicles.filter(v=>v.clientId===item.id&&canAccess(user,v.ownerId,'vehicles','edit')).forEach(v=>Object.assign(v,{company:item.name||'',ownerName:item.name||'',document:item.document||'',contact:item.contact||'',phone:item.phone||'',email:item.email||'',city:item.city||'',state:item.state||'',clientName:item.name||'',updatedAt:now()}));audit(user,`editou registro em ${collection}`,item.name||item.description||item.id);saveDB(db);return json(res,200,item);}
